...
export const KPIs: React.FC<KPIProps> = ({ stats, setActiveTab }) => {
  const kpis = React.useMemo(() => [
    ...
    { label: 'Estoque Baixo', value: stats.lowStock, icon: AlertCircle, color: stats.lowStock > 0 ? 'text-rose-500' : 'text-slate-400', bg: stats.lowStock > 0 ? 'bg-rose-500/10' : 'bg-slate-500/10', link: 'inventory' },
  ], [stats]);

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
      {kpis.map((kpi, i) => (
        <Card key={i} className="group !p-8 hover:translate-y-[-4px] transition-all duration-500">
          <div className="flex items-center justify-between mb-8">
            ...
            <Button 
              ...
              className="w-12 h-12 !p-0 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-300 group-hover:text-accent group-hover:bg-accent/10 transition-all"
            >
              <ArrowUpRight size={24} />
            </Button>
          </div>
          <div>
            <p className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-3">{kpi.label}</p>
            <h3 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tight font-display">{kpi.value}</h3>
          </div>
        </Card>
      ))}
    </div>
