...
const FinancialDashboard: React.FC = () => {
  ...
  const currentMonth = useMemo(() => {
    ...
    const monthOS = serviceOrders.filter(os => {
      const date = new Date(os.updatedAt || os.createdAt);
  ...
  const stats = [
    ...
    {
      label: 'Inadimplência',
      value: formatCurrency(transactions.filter(t => t.status === 'pending' && t.type === 'in' && new Date(t.dueDate || '') < new Date()).reduce((acc, t) => acc + t.value, 0)),
      icon: AlertCircle,
      color: 'text-red-600',
      bg: 'bg-red-50',
      trend: 'Total vencido'
  ...
  if (loading) return <div className="p-8 text-center text-zinc-400 italic">Carregando dashboard...</div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white p-6 rounded-[2rem] border border-zinc-200 shadow-sm hover:shadow-md transition-all">
            ...
            <div className="pt-4 border-t border-zinc-50">
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">{stat.trend}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h4 className="text-lg font-black text-zinc-900">Desempenho Financeiro</h4>
              <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">Últimos 6 meses</p>
            </div>
          </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h4 className="text-lg font-black text-zinc-900">Metas do Mês</h4>
              <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">Progresso Atual</p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            {currentMonth.goal ? (
              <>
                <div className="space-y-3">
                  <div className="flex justify-between text-xs font-bold uppercase tracking-widest">
                    ...
                    <span className="text-zinc-900">{Math.round((currentMonth.income / currentMonth.goal.faturamento) * 100)}%</span>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-xs font-bold uppercase tracking-widest">
                    ...
                    <span className="text-zinc-900">{Math.round((currentMonth.profit / currentMonth.goal.lucroLiquido) * 100)}%</span>
                  </div>
                  <div className="h-3 bg-zinc-100 rounded-full overflow-hidden">
                    <div 
              ...
              <div className="py-12 text-center">
                <p className="text-sm text-zinc-400 italic">Nenhuma meta definida para este mês.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-zinc-200 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-zinc-100">
          <h4 className="text-lg font-black text-zinc-900">Rentabilidade por OS</h4>
          <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">Análise de Margem de Contribuição</p>
        </div>
            ...
            <tbody className="divide-y divide-zinc-50">
              {serviceOrders
                ...
                .map((os) => {
                  ...
                  return (
                    <tr key={os.id} className="hover:bg-zinc-50 transition-all group">
                      <td className="px-8 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-zinc-900">OS #{os.numeroOS || os.id.slice(-4).toUpperCase()}</span>
                          <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">
                            {format(new Date(os.updatedAt || os.createdAt), 'dd/MM/yyyy')}
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-4 text-right text-sm font-bold text-zinc-900">{formatCurrency(revenue)}</td>
                      ...
                      <td className="px-8 py-4 text-right">
                        <div className="inline-flex items-center gap-1 px-3 py-1 bg-zinc-100 text-zinc-900 rounded-full text-[10px] font-black uppercase tracking-widest">
                          {Math.round(margin)}%
                        </div>
                      </td>
                    </tr>
