    ...
    <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">Análise de recorrência e valor</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Clientes Recorrentes (Top 10)</h4>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recurring.map((client, i) => (
              <div key={client.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all group">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    <h5 className="text-sm font-black text-slate-900 tracking-tight">{client.name}</h5>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{client.visitCount} visitas</p>
                  </div>
                </div>
                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Gasto</p>
                  <p className="text-sm font-black text-slate-900">{formatCurrency(client.totalSpent)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-8">
          <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl shadow-slate-200">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Resumo de Retenção</h4>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Curva ABC (Top Clientes)</p>
                <div className="space-y-3 mt-4">
                  {['A', 'B', 'C'].map(cat => {
                    const count = abc.filter(c => c.category === cat).length;
                    const total = abc.length;
                    const percentage = total > 0 ? (count / total) * 100 : 0;
                    const color = cat === 'A' ? 'bg-emerald-500' : cat === 'B' ? 'bg-blue-500' : 'bg-slate-500';
                    return (
